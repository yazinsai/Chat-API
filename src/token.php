<?php

require_once 'func.php';

/**
 * We need to update the token, as it's currently failing with the code "bad_token"
 * I imagine we'll need to extract this from the APK in order to use the API (again,
 * assuming the API hasn't itself changed -- which is likely to be the case since
 * we can interact with it)
 * 
 * Static fields:
 * - $signature
 * 
 * These fields change between releases:
 * - $classesMd5: MD5 hash of the 'classes.dex' file
 * - $key2: Precalculated value for the key. Check README for reference
 */

function generateRequestToken($country, $phone, $platform)
{
    switch ($platform) {
    case 'Android':
      // com.google.android.search.verification.client.SearchActionVerificationClientUtil.class -> isPackageGoogleSigned @ bool2
      $signature = '';
      $classesMd5 = 'p1J+2B1aBTuUMhoDkoECBA=='; // 2.19.81

      $key2 = base64_decode('eQV5aq/Cg63Gsq1sshN9T3gh+UUp0wIw0xgHYT1bnCjEqOJQKCRrWxdAe2yvsDeCJL+Y4G3PRD2HUF7oUgiGo8vGlNJOaux26k+A2F3hj8A=');
      $data = base64_decode($signature).base64_decode($classesMd5).$phone;

      $opad = str_repeat(chr(0x5C), 64);
      $ipad = str_repeat(chr(0x36), 64);
      for ($i = 0; $i < 64; $i++) {
          $opad[$i] = $opad[$i] ^ $key2[$i];
          $ipad[$i] = $ipad[$i] ^ $key2[$i];
      }

      $output = hash('sha1', $opad.hash('sha1', $ipad.$data, true), true);

      return base64_encode($output);

    case 'Nokia':
      $const = 'PdA2DJyKoUrwLw1Bg6EIhzh502dF9noR9uFCllGk';
      $releaseTime = '1452554789539'; // 2.13.30
      $token = md5($const.$releaseTime.$phone);

      return $token;
  }
}
